...
For another, when one target's selection depends on the properties
...
For yet another, when a survey is beautifully targetted, but has low
\foreign{a posteriori} success rate on its targets, the selection
function might depend on properties of the sources that are hard to
understand or model.

In this \documentname, we develop guidelines for target selection
that, if followed, will improve the chances that it will be possible
to build a simple and accurate selection function.
...
or the density of stars as a function of radius in the Milky Way disk;
...
We will also assume (in this \documentname) that the survey is making
single visits, or multiple visits without adapting to prior
spectroscopic results.
That is, our discussion will not apply very usefully to adaptive
time-domain surveys, which take some spectroscopic data and use that
data to decide whether or not to take more data.
This makes our discussion less useful to those following up exoplanet
...
As we say above, we imagine that we are designing a spectroscopic project
with population-level statistical goals, for example XXX or YYY.
However the data analysis is going to proceed, it is useful to think in
