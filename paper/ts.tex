...
% - make sure to cite exopop, loredo, and blogpost.
% - ...
...
This population model $p_0(\set{z_m}\given\theta)$ could have any arbitrarily
complicated form.
...
\begin{align}
    p_0(\set{z_m}\given\theta) &= \exp -Q(\theta)\,\prod_m p_0(z_m\given\theta) \\
\ln p_0(\set{z_m}\given\theta) &= \sum_m \ln p_0(z_m\given\theta) - Q(\theta)
\end{align}
...
models $p_0(z_m\given\theta)$ represent the pdfs for individual
...
works from the true properties $z_m$ to the observed values $y_m$.
If this model contains uncertain parameters (unknown gains, say, or
unknown read noise, or \foreign{etc.}), then this function
$p(y_m\given z_m)$ could be conditioned on additional noise-model
...
$p(\set{y_m}\given\set{z_m})$,
we have assumed that the observed properties $y_m$ of source $m$
depend only on the true properties $z_m$ \emph{of that source}.
...
\bibitem[Loredo \& Wasserman(1995)]{loredo} Loredo, T.~J., \& Wasserman, I.~M., 1995, \apjs, 96, 261 
\end{thebibliography}
