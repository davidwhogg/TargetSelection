...
HOGG SYNTACTICAL; IN THE MIDDLE OF RIX COMMENTS.

The whole experiment results in a catalog of objects, $\set{x_n}_{n=1}^N$, where the $x_n$ are
the data available on each object, comprising in our
example both estimates of physical quantities derived from the spectra
(spectroscopic parallaxes, logg, Teff, vrad, etc..) and the
information used in making the decision whether to target the object
in the first place (position, magnitude, color, etc..). The catalog
columns x may also contain quantities that neither entered the
targetting decision nor were derived from the spectra, but may prove
useful in the subsequent analysis, such as proper motions.
